...
export const adjustTo: IAdjustTo =
  ...
  (target, adjust, move = {}) => {
    // willContain = true: target 是 checker 的子节点，不受 checker 的 scale 影响
    // willContain = false: target 与 checker 同级，需要考虑 checker 的 scale
    const checkWidth = willContain
      ? checker.width
      : checker.width * checker.scaleX;
    const checkHeight = willContain
      ? checker.height
      : checker.height * checker.scaleY;
